...
// path index → gu name (SVG 파일 순서 기준)
export const SEOUL_GU_PATHS = {
...
// 전국 지도에서 서울을 색칠할 대표 SVG 번호
export const SEOUL_REPRESENTATIVE_SVG = 227

export const searchGus = (query) => {
  const q = query.trim()
  if (!q) return []
  return SEOUL_GU_NAMES.filter(name => name.includes(q))
}
